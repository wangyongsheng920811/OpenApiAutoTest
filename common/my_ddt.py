...
def file_data(common_test=0):
    ...
    def wrapper(func):
        ...
        if 'prod' in sys.argv[:]:
            ...
            f = '.json'
        value = file_path.replace(os.sep + 'case' + os.sep, os.sep + 'data' + os.sep).replace('.py', f)
...
def format_case_name(name):
    global CASE_NUMBER
    name = 'test_{:0>4d}'.format(CASE_NUMBER) + name[4:]
    ...
    with codecs.open(data_file_path, 'r', 'utf-8') as f:
        ...
        if _is_yaml_file:
            ...
            if data:
                ...
                test_name = mk_test_name(name, 'error')
                test_docstring = 'Error!'
                add_test(cls, test_name, test_docstring, create_error_func('%s case name :' + name + ' not found in json'), None)
                return
        ...
        if isinstance(data, dict):
            ...
            for i, elem in enumerate(case_data):
                ...
                if 'test_common' in key:
                    test_name = format_case_name(name) + '_' + case_name + '_' + key
                    if isinstance(value, dict):
                        value.update({'path': path})
                        add_test(cls, test_name, test_desc, func, **value)
                    else:
...
def ddt(cls):
    ...
    for name, func in list(cls.__dict__.items()):
        if hasattr(func, DATA_ATTR):
            ...
            delattr(cls, name)
    CASE_NAMES = [case for case in dir(cls) if 'test_0' in case]
    setattr(cls, 'CASE_NAMES', CASE_NAMES)
    return cls
